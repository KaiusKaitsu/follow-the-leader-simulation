...
class Simulant(isLeader: Boolean):
  ...
  /** logic to enable kinetics between the simulants and also to make them unable to be all over each other */
  private def collisionCheck(newPosition: Vector2D, newMomentum: Vector2D) =
    ...
    for (sim <- simulants) do
      ...
      if distance < 10 then //collision checker. Simulants radius is 5
